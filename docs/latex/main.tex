...
\begin{enumerate}[leftmargin=1.5em]
    ...
    \item text-prompt detection over those frames using a Grounding DINO-based step,
...
\subsubsection{Detector benchmark: Grounding DINO versus YOLO-World}

Although the first operational version of the preprocessing stack was built around Grounding DINO, the architecture deliberately keeps the detector stage replaceable. Before freezing the next iteration of the pipeline, a dedicated benchmark was therefore carried out to compare two candidate detectors: \texttt{IDEA-Research/grounding-dino-tiny} and \texttt{yolov8s-worldv2.pt} (YOLO-World). The purpose of this benchmark was not to claim a universally better detector, but to characterise the engineering trade-off that matters most for the present system: preprocessing time versus detection coverage. Because AOIs are produced offline, a faster detector can reduce turnaround time during stimulus preparation and debugging. However, if that speed gain is achieved at the cost of missing too many relevant objects, the quality of exported AOI maps may degrade and later gaze--AOI analyses may become less reliable.

The benchmark was conducted as a technical comparison rather than as the final scientific validation corpus of the study. Five 360\textdegree\ video clips were used as an engineering benchmark set: \texttt{sample2Camera360}, \texttt{sampleCamera360}, \texttt{sampleForest360}, \texttt{sampleLions360}, and \texttt{sampleWolves360}. For each video, the benchmark consumed the pre-extracted sparse frame folders already generated by the preprocessing pipeline. Thirty frames were sampled evenly across each video, one warm-up run was executed per model and video to avoid counting cold-start effects, and two measured runs were retained in the final summary. Both detectors were executed on the same hardware configuration, namely an NVIDIA GeForce RTX 3050 Laptop GPU with CUDA~12.6 and \texttt{torch 2.11.0+cu126}, using batch size~4 and FP16 inference. Grounding DINO additionally downscaled frames for inference to at most 1920$\times$960, whereas YOLO-World used the same sparse frames with the fixed prompt classes \texttt{person}, \texttt{face}, \texttt{bottle}, \texttt{screen}, and \texttt{product}. Warm-up runs were excluded from the reported statistics.

Table~\ref{tab:detector-benchmark-summary} and Figure~\ref{fig:detector-benchmark} summarise the main outcome of the comparison. YOLO-World was faster than Grounding DINO in all five videos. The observed speed-up ranged from 7.73$\times$ to 14.26$\times$, with a mean acceleration of approximately 10.15$\times$. This result is large enough to matter operationally: a detector that consistently reduces preprocessing time by one order of magnitude makes iterative AOI generation, parameter tuning, and pipeline debugging substantially easier.

The other side of the trade-off is detection coverage. Across the five benchmark videos, YOLO-World produced on average 48\% of the detections returned by Grounding DINO. The loss was moderate in \texttt{sample2Camera360} and \texttt{sampleForest360}, where YOLO-World retained roughly three quarters of the detections. It became more pronounced in \texttt{sampleCamera360} and especially \texttt{sampleLions360}. The most extreme case was \texttt{sampleWolves360}, where Grounding DINO still produced 16 detections while YOLO-World returned none. This pattern suggests that the gain in throughput is accompanied by a reduction in recall under the fixed prompt set and confidence configuration used in the benchmark.

The most plausible explanation is architectural. Grounding DINO is a heavier language-grounded transformer detector and is therefore computationally more expensive, but it appears to preserve better coverage on sparse, ambiguous, or visually difficult frames. YOLO-World, by contrast, benefits from a much lighter detection path and therefore offers far lower latency, but it seems more sensitive to the exact prompt vocabulary, detector confidence, and appearance variation across scenes. In other words, the benchmark does not suggest that YOLO-World is ``wrong'' in general; rather, it shows that under the present settings it trades recall for speed. This interpretation is consistent with the fact that YOLO-World remained fast and stable across all videos while the largest detection losses occurred in the clips that already looked challenging from a preprocessing perspective.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lrrrrrr}
        \toprule
        Stimulus & GD time (s) & YW time (s) & Speed-up & GD det. & YW det. & YW/GD \\
        \midrule
        \texttt{sample2Camera360} & 67.75 & 6.75 & 10.04 & 236 & 174 & 0.74 \\
        \texttt{sampleCamera360} & 69.38 & 8.97 & 7.73 & 459 & 271 & 0.59 \\
        \texttt{sampleForest360} & 75.55 & 7.16 & 10.55 & 45 & 34 & 0.76 \\
        \texttt{sampleLions360} & 54.92 & 6.71 & 8.19 & 34 & 10 & 0.29 \\
        \texttt{sampleWolves360} & 106.59 & 7.48 & 14.26 & 16 & 0 & 0.00 \\
        \bottomrule
    \end{tabular}
    \caption{Mean benchmark results over two measured runs per detector and video. GD = Grounding DINO, YW = YOLO-World.}
    ...
    \begin{subfigure}{\textwidth}
        ...
        \begin{tikzpicture}
            \begin{axis}[
                ...
                legend style={font=\small,at={(0.5,1.05)},anchor=south,legend columns=2},
                ymajorgrids=true,
                grid style={dashed,gray!30}
            ]
                \addplot coordinates {(v1,67.75) (v2,69.38) (v3,75.55) (v4,54.92) (v5,106.59)};
                \addplot coordinates {(v1,6.75) (v2,8.97) (v3,7.16) (v4,6.71) (v5,7.48)};
                \legend{Grounding DINO, YOLO-World}
            \end{axis}
        \end{tikzpicture}
    ...
    \begin{subfigure}{\textwidth}
        ...
        \begin{tikzpicture}
            \begin{axis}[
                ...
                \addplot coordinates {(v1,236) (v2,459) (v3,45) (v4,34) (v5,16)};
                \addplot coordinates {(v1,174) (v2,271) (v3,34) (v4,10) (v5,0)};
                \legend{Grounding DINO, YOLO-World}
            \end{axis}
        \end{tikzpicture}
        \caption{Mean number of exported detections per video.}
    \end{subfigure}
    \caption{Detector benchmark on the five-video engineering benchmark set. The upper chart shows the strong runtime advantage of YOLO-World, whereas the lower chart shows the corresponding drop in detection yield on several videos.}
    \label{fig:detector-benchmark}
\end{figure}

From the standpoint of this project, the benchmark leads to two practical conclusions. First, the detector stage should remain configurable rather than hard-coded, because the ``best'' model depends on whether the current priority is speed or coverage. Second, the final AOI generation workflow should not rely exclusively on raw detection counts when selecting a model. A faster detector is useful only insofar as the resulting AOIs remain spatially and functionally valid for later gaze analysis. For that reason, the benchmark reported here is best understood as an engineering filter for the preprocessing stage, not as a substitute for the later validation against manual annotations and downstream gaze metrics. In practical terms, the current evidence supports using YOLO-World for rapid exploratory preprocessing and iteration, while keeping Grounding DINO as the safer option when difficult stimuli require higher detection coverage before the AOI sequence is frozen.
...
In the current prototype, CSV export is fixation-based rather than raw-sample-complete: the runtime commits fixation events every 250 ms and writes AOI-linked rows at that cadence. This choice was made to validate the end-to-end integration path early while keeping runtime logging volume manageable during device-side testing. The broader analytical design, however, still assumes a future separation between raw gaze samples and derived fixation events when the final experimental protocol is frozen.
...
Accordingly, the current result of this phase is the establishment of a reproducible analysis contract. The essential scientific question remains open and will be answered in the next phase: whether substituting manually annotated AOIs with automatically generated AOIs materially changes the interpretation of gaze behaviour.
